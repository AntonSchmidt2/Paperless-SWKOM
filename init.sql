...
CREATE TABLE documents_correspondent (
     ...
     match TEXT NOT NULL,
     matching_algorithm INT NOT NULL CHECK (matching_algorithm >= 0),
...
alter table documents_document
    owner to postgres;
ALTER TABLE documents_document
    ALTER COLUMN checksum SET DEFAULT 'default_value';
...
-- paperless_mail_mailrule table
-- Correcting the 'order' column which is a reserved keyword in SQL
CREATE TABLE paperless_mail_mailrule (
     ...
     maximum_age INT NOT NULL CHECK (maximum_age >= 0),
     action INT NOT NULL CHECK (action >= 0),
     action_parameter VARCHAR(256),
     assign_title_from INT NOT NULL CHECK (assign_title_from >= 0),
     assign_correspondent_from INT NOT NULL CHECK (assign_correspondent_from >= 0),
     account_id INT NOT NULL REFERENCES paperless_mail_mailaccount(id),
     assign_correspondent_id INT REFERENCES documents_correspondent(id),
     assign_document_type_id INT REFERENCES documents_documenttype(id),
     "order" INT NOT NULL, -- Enclosed in quotes
     attachment_type INT NOT NULL CHECK (attachment_type >= 0),
     filter_attachment_filename VARCHAR(256),
     consumption_scope INT NOT NULL CHECK (consumption_scope >= 0),
     owner_id INT REFERENCES auth_user(id),
